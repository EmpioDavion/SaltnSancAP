...
def create_event_item(player: int, name: str) -> Item:
    return items.SaltnSancItem(name, ItemClassification.progression, None, player)


def create_location(player: int, name: str, region: Region) -> Location:
    return locations.SaltnSancLocation(player, name, None, region)
...
def create_all_events(world: "SaltnSancWorld", created_regions: dict[str, Region]) -> None:
